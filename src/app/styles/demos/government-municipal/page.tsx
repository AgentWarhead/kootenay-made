    ...
    <div className="flex flex-col gap-3">
      {items.map((item, i) => (
        <div key={i} className="rounded-lg overflow-hidden transition-shadow"
          style={{ backgroundColor: hc ? '#ffffff' : '#ffffff', border: hc ? '2px solid #000000' : '1px solid #e2e8f0', boxShadow: hc ? 'none' : open === i ? '0 4px 16px rgba(37,99,235,0.08)' : 'none', borderLeftWidth: open === i ? '4px' : hc ? '2px' : '1px', borderLeftColor: open === i ? blue : hc ? '#000000' : '#e2e8f0' }}>
          <button onClick={() => setOpen(open === i ? null : i)}
            className="gov-focusable w-full flex items-center justify-between px-6 py-5 text-left"
            aria-expanded={open === i}
            style={{ minHeight: '44px' }}>
            <span className="text-base font-600" style={{ color: open === i ? blue : navy }}>{item.q}</span>
          ...
          {open === i && (
            <div className="px-6 pb-5 text-base leading-relaxed" style={{ color: '#475569' }}>{item.a}</div>
          )}
        </div>
      ))}
    </div>
    ...
    <div className={sourceSans.className} style={{ backgroundColor: C.pageBg, color: C.navy, minHeight: '100vh' }}>
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between h-16 gap-4">
          ...
          <div className="hidden md:flex items-center gap-6">
            {['Programs', 'Facilities', 'About', 'Contact'].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-600 transition-opacity gov-focusable"
            ...
            <a href="tel:2505550116" className="text-sm font-700 gov-focusable" style={{ color: C.navText, textDecoration: C.linkDecor }}>(250) 555-0116</a>
          </div>
          <div className="flex items-center gap-3">
            ...
            <a href="tel:2505550116" className="md:hidden text-sm font-700 gov-focusable" style={{ color: C.navText, textDecoration: C.linkDecor }}>Call</a>
          </div>
        </div>
      </nav>
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        <section className="relative overflow-hidden min-h-screen flex items-center"
          style={{ background: hc ? '#000000' : 'linear-gradient(165deg, #1e3a5f 0%, #162d4a 100%)' }}
          ...
          <div className="relative z-10 max-w-5xl mx-auto px-6 py-28 md:py-36 w-full">
            <motion.div className="mb-12 max-w-2xl mx-auto"
              ...
              <div className="flex items-center rounded-lg overflow-hidden" style={{ backgroundColor: '#ffffff', boxShadow: '0 4px 24px rgba(0,0,0,0.25)' }}>
                <label htmlFor="hero-search" className="sr-only">Search programs, facilities, hours</label>
                <input id="hero-search" type="search" placeholder="Search programs, facilities, hours..."
                  className="gov-focusable flex-1 px-5 py-4 text-base outline-none"
                  style={{ color: '#1e3a5f', backgroundColor: 'transparent', border: 'none', fontSize: '1rem' }} />
                <button aria-label="Submit search" className="gov-focusable px-5 py-4 flex items-center justify-center transition-colors"
                  style={{ backgroundColor: '#2563eb', color: '#ffffff', border: 'none', cursor: 'pointer', minWidth: '56px', minHeight: '56px' }}
                  onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = '#1d4ed8')}
                  onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = '#2563eb')}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>
                </button>
              </div>
            </motion.div>
            <div className="flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-12">
              <div className="flex-1 text-center md:text-left">
                <motion.h1 className="text-5xl sm:text-6xl md:text-7xl font-700 leading-tight mb-6"
                ...
                <motion.p className="text-lg md:text-xl max-w-xl mb-10 leading-relaxed"
                  style={{ color: hc ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.78)' }}
                  initial={prefersReduced ? {} : { opacity: 0, y: 28 }}
                  animate={prefersReduced ? {} : { opacity: 1, y: 0 }}
                  transition={{ duration: 0.65, delay: 0.5 }}>
                  Discover trails, sports facilities, aquatic centres, and hundreds of recreation programs for all ages and abilities across the West Kootenays.
                </motion.p>
                <motion.div initial={prefersReduced ? {} : { opacity: 0, y: 20 }} animate={prefersReduced ? {} : { opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.7 }}>
                  <a href="#programs" className="gov-focusable inline-block px-10 font-700 text-base transition-colors rounded-lg"
                    style={{ backgroundColor: hc ? '#ffffff' : '#2563eb', color: hc ? '#000000' : '#ffffff', textDecoration: C.linkDecor, border: hc ? '2px solid #ffffff' : 'none', minHeight: '52px', lineHeight: '52px', boxShadow: hc ? 'none' : '0 4px 20px rgba(37,99,235,0.45)' }}
                    onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = hc ? '#e0e0e0' : '#1d4ed8')}
                    onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = hc ? '#ffffff' : '#2563eb')}>
                    View All Programs
                  </a>
                </motion.div>
              </div>
              <div className="flex-shrink-0"><OfficialSeal hc={hc} /></div>
            </div>
          </div>
        </section>
        ...
        {/* ═══ 4. SERVICES ═══ */}
        <section id="programs" className="py-20 md:py-28 px-6" style={{ backgroundColor: C.sectionAlt }} aria-labelledby="services-heading">
          <div className="max-w-6xl mx-auto">
            <Reveal className="text-center mb-10">
              <p className="text-sm font-700 uppercase tracking-widest mb-3" style={{ color: C.blue, letterSpacing: '0.15em' }}>Digital Services</p>
              <h2 id="services-heading" className="text-3xl md:text-4xl font-700" style={{ color: C.navy }}>Better Serving Our Residents</h2>
            </Reveal>

            {/* PAS Copy */}
            <Reveal delay={0.05} className="text-center mb-14">
              <p className="text-base md:text-lg leading-relaxed max-w-2xl mx-auto" style={{ color: C.slate }}>
                Residents can&rsquo;t find basic info and call the office for everything — program hours, pool schedules, trail closures. Your website should answer their questions before they pick up the phone. The municipality down the highway redesigned their site and cut call volume by 40%.{' '}
                <span style={{ color: C.blue, fontWeight: 700 }}>Yours can too.</span>
              </p>
            </Reveal>
              ...
              {[
                {
                  ...
                  desc: 'A clear, accessible site residents can use to find programs, hours, and contact info — without calling. From $1,500.',
                  icon: <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={hc ? '#000000' : '#2563eb'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="2" y="3" width="20" height="14" rx="2" /><line x1="8" y1="21" x2="16" y2="21" /><line x1="12" y1="17" x2="12" y2="21" /></svg>,
                },
                {
                  title: 'Email Marketing',
                  desc: 'Keep residents informed about events, closures, and new programs automatically. From $750.',
                  icon: <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={hc ? '#000000' : '#2563eb'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="2" y="4" width="20" height="16" rx="2" /><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" /></svg>,
                },
                {
                  title: 'Google Visibility',
                  desc: 'Make sure residents can find facilities the moment they search. Google Domination from $500.',
                  icon: <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={hc ? '#000000' : '#2563eb'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>,
                },
              ].map((card, i) => (
                <Reveal key={card.title} delay={i * 0.12}>
                  <div className="p-8 rounded-lg transition-shadow"
                    ...
                    onMouseEnter={(e) => { if (!hc) e.currentTarget.style.boxShadow = '0 8px 28px rgba(37,99,235,0.12)' }}
                    onMouseLeave={(e) => { if (!hc) e.currentTarget.style.boxShadow = '0 2px 12px rgba(0,0,0,0.06)' }}>
                    ...
                    <h3 className="text-xl font-700 mb-3" style={{ color: C.navy }}>{card.title}</h3>
                    <p className="text-base leading-relaxed" style={{ color: C.slate }}>{card.desc}</p>
                  </div>
                </Reveal>
              ))}
            </div>
        ...
        <section className="py-20 md:py-28 px-6" style={{ backgroundColor: C.white }} aria-labelledby="hiw-heading">
          <div className="max-w-5xl mx-auto">
            <Reveal className="text-center mb-14">
              <p className="text-sm font-700 uppercase tracking-widest mb-3" style={{ color: C.blue, letterSpacing: '0.15em' }}>The Process</p>
              <h2 id="hiw-heading" className="text-3xl md:text-4xl font-700" style={{ color: C.navy }}>How It Works</h2>
              <p className="mt-4 max-w-xl mx-auto text-base" style={{ color: C.slateLight }}>Simple, transparent, no surprises. Just like good government should be.</p>
            </Reveal>
              ...
              {[
                ...
                <Reveal key={step.num} delay={i * 0.15}>
                  <div className="flex gap-6 items-start">
                    <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 font-700 text-xl text-white"
                      style={{ backgroundColor: C.blue, border: hc ? '2px solid #000000' : 'none', minWidth: '48px' }}>{step.num}</div>
                    <div>
                      <h3 className="text-xl font-700 mb-2" style={{ color: C.navy }}>{step.title}</h3>
                      <p className="text-base leading-relaxed" style={{ color: C.slate }}>{step.desc}</p>
                    </div>
                  </div>
                </Reveal>
              ))}
        ...
        {/* ═══ 5. GALLERY ═══ */}
        <section className="py-20 md:py-28 px-6" style={{ backgroundColor: C.sectionAlt }} aria-labelledby="gallery-heading">
          <div className="max-w-6xl mx-auto">
            <Reveal className="text-center mb-12">
              <h2 id="gallery-heading" className="text-3xl md:text-4xl font-700" style={{ color: C.navy }}>Our Parks &amp; Facilities</h2>
              <p className="mt-4 max-w-xl mx-auto text-base" style={{ color: C.slateLight }}>From trail networks to indoor recreation centres — there&rsquo;s something for everyone.</p>
            </Reveal>
        ...
        {/* ═══ TESTIMONIALS (3) ═══ */}
        <section className="py-20 md:py-28 px-6" style={{ backgroundColor: C.sectionAlt }} aria-labelledby="testimonials-heading">
          <div className="max-w-6xl mx-auto">
            <Reveal className="text-center mb-14">
              <p className="text-sm font-700 uppercase tracking-widest mb-3" style={{ color: C.blue, letterSpacing: '0.15em' }}>Community Feedback</p>
              <h2 id="testimonials-heading" className="text-3xl md:text-4xl font-700" style={{ color: C.navy }}>What Residents Say</h2>
            </Reveal>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { quote: 'I can finally find pool schedules and program registration without calling the office. I sent the link to my whole neighbourhood.', name: 'Sandra K.', org: 'Resident', town: 'Trail' },
                ...
                { quote: 'We asked for something accessible and modern. What we got was exactly that — and our residents noticed immediately. Complaints about the website basically stopped.', name: 'Pam T.', org: 'District Administrator', town: 'Nakusp' },
              ].map((t, i) => (
                <Reveal key={i} delay={i * 0.12}>
                  <div className="p-10 rounded-lg"
                    style={{ backgroundColor: C.white, borderLeft: hc ? '4px solid #000000' : '4px solid #2563eb', border: hc ? '2px solid #000000' : undefined, boxShadow: hc ? 'none' : '0 4px 24px rgba(0,0,0,0.06)' }}>
                    ...
                    <blockquote>
                      <p className="text-base font-600 leading-relaxed mb-5" style={{ color: C.navy, fontStyle: 'italic' }}>&ldquo;{t.quote}&rdquo;</p>
                      <footer>
                        <cite className="text-sm font-700 not-italic" style={{ color: C.blue }}>— {t.name}</cite>
        ...
        {/* ═══ FAQ ═══ */}
        <section className="py-20 md:py-28 px-6" style={{ backgroundColor: C.white }} aria-labelledby="faq-heading">
          <div className="max-w-3xl mx-auto">
            <Reveal className="text-center mb-14">
              <p className="text-sm font-700 uppercase tracking-widest mb-3" style={{ color: C.blue, letterSpacing: '0.15em' }}>Questions &amp; Answers</p>
              <h2 id="faq-heading" className="text-3xl md:text-4xl font-700" style={{ color: C.navy }}>Common Questions</h2>
            </Reveal>
            <Reveal delay={0.1}>
              <FAQAccordion hc={hc} items={[
                { q: 'How long does a website take?', a: 'Typically 2–3 weeks from our first meeting to launch. Municipal sites often need additional review cycles — we factor that in and keep you on schedule.' },
                { q: 'Does it meet WCAG accessibility standards?', a: 'Yes. We build to WCAG 2.1 AA standards as a baseline — proper contrast ratios, keyboard navigation, skip links, aria labels, and screen reader compatibility. We also include a built-in high contrast mode.' },
                { q: 'Can it host documents, PDFs, and bylaws?', a: 'Yes. We build structured document libraries so residents can find agendas, bylaws, permits, and forms without calling the office.' },
                { q: 'What does it cost?', a: 'Custom government and municipal websites start from $1,500. A full brand package starts from $4,000. Book a free consultation and we\'ll quote your specific needs.' },
                { q: 'Can we update it ourselves after launch?', a: 'Yes. We build on platforms your staff can manage — adding events, updating hours, uploading documents — without needing a developer.' },
                { q: 'What if we already have a website?', a: 'We rebuild it to modern standards, migrating your content, documents, and structure. Your old site stays live until the new one is ready.' },
              ]} />
            </Reveal>
          </div>
        </section>
        ...
        {/* ═══ 7. ABOUT ═══ */}
        <section id="about" className="py-20 md:py-28 px-6" style={{ backgroundColor: C.sectionAlt }} aria-labelledby="about-heading">
          <div className="max-w-5xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-14 items-center">
              <Reveal>
                <p className="text-sm font-700 uppercase tracking-widest mb-4" style={{ color: C.blue, letterSpacing: '0.15em' }}>About Us</p>
                <h2 id="about-heading" className="text-3xl md:text-4xl font-700 mb-6" style={{ color: C.navy }}>Committed to Our Communities Since 1980</h2>
                ...
                <p className="text-base leading-relaxed mb-8" style={{ color: C.slate }}>
                  We believe that access to recreation is a right, not a privilege. Our programs are designed with accessibility in mind — including adaptive programming, seniors&rsquo; fitness, youth leadership, and everything in between.
                </p>
                <a href="#contact" className="gov-focusable inline-block px-8 font-700 text-base transition-colors rounded-lg"
                ...
                <div className="grid grid-cols-2 gap-5">
                  {[{ num: '45+', label: 'Parks & Trails' }, { num: '12', label: 'Facilities' }, { num: '200+', label: 'Programs' }, { num: '50K+', label: 'Annual Visits' }].map((stat) => (
                    <div key={stat.label} className="flex flex-col items-center justify-center p-6 text-center rounded-lg"
                      style={{ backgroundColor: C.white, border: hc ? '2px solid #000000' : '1px solid #e2e8f0' }}>
                      <div className="text-3xl font-700 mb-1" style={{ color: C.blue }}>{stat.num}</div>
                      <div className="text-xs font-600 uppercase tracking-wider" style={{ color: C.slateLight }}>{stat.label}</div>
                    </div>
                  ))}
                </div>
              </Reveal>
        ...
        {/* ═══ 8. CONTACT ═══ */}
        <section id="contact" className="py-20 md:py-28 px-6" style={{ backgroundColor: C.white }} aria-labelledby="contact-heading">
          <div className="max-w-6xl mx-auto">
            <Reveal className="text-center mb-14">
              <p className="text-sm font-700 uppercase tracking-widest mb-3" style={{ color: C.blue, letterSpacing: '0.15em' }}>Get in Touch</p>
              <h2 id="contact-heading" className="text-3xl md:text-4xl font-700" style={{ color: C.navy }}>Contact West Kootenay Parks &amp; Rec</h2>
            </Reveal>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
              <Reveal>
                <div className="flex flex-col gap-7">
                  {[
                    { label: 'Phone', value: '(250) 555-0116', href: 'tel:2505550116', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={hc ? '#000000' : '#2563eb'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.12 13 19.79 19.79 0 0 1 1.07 4.4 2 2 0 0 1 3.05 2h3a2 2 0 0 1 2 1.72c.13.96.36 1.9.7 2.81a2 2 0 0 1-.45 2.11L7.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.91.34 1.85.57 2.81.7A2 2 0 0 1 21 16.92z" /></svg> },
                    { label: 'Email', value: 'info@wkparks.ca', href: 'mailto:info@wkparks.ca', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={hc ? '#000000' : '#2563eb'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="2" y="4" width="20" height="16" rx="2" /><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" /></svg> },
                    { label: 'Hours', value: '7 Days a Week, 8:00 AM – 6:00 PM', href: null, icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={hc ? '#000000' : '#2563eb'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg> },
                    { label: 'Locations', value: 'Nelson · Trail · Castlegar · Rossland · Nakusp', href: null, icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={hc ? '#000000' : '#2563eb'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg> },
                  ].map((item) => (
                    <div key={item.label} className="flex items-start gap-4">
                      <div className="w-11 h-11 rounded-lg flex items-center justify-center flex-shrink-0"
                        style={{ backgroundColor: hc ? '#e0e0e0' : '#dbeafe', border: hc ? '2px solid #000000' : 'none', minWidth: '44px', minHeight: '44px' }}>{item.icon}</div>
                      <div>
                        <div className="text-xs font-700 uppercase tracking-wider mb-0.5" style={{ color: C.slateLight }}>{item.label}</div>
                        {item.href ? (
                          <a href={item.href} className="gov-focusable font-600 text-base" style={{ color: C.navy, textDecoration: hc ? 'underline' : 'none' }}>{item.value}</a>
                        ) : (
                          <span className="font-600 text-base" style={{ color: C.navy }}>{item.value}</span>
                        )}
                      </div>
                    </div>
                  ))}
              ...
              <Reveal delay={0.15}>
                <form onSubmit={(e) => e.preventDefault()} noValidate aria-label="Contact form"
                  style={{ backgroundColor: C.sectionAlt, border: hc ? '2px solid #000000' : '1px solid #e2e8f0', borderRadius: '8px', padding: '2rem', boxShadow: hc ? 'none' : '0 4px 20px rgba(0,0,0,0.06)' }}>
                  <div className="flex flex-col gap-5">
                    {[{ id: 'contact-name', label: 'Your Name', type: 'text', placeholder: 'Jane Smith', auto: 'name' }, { id: 'contact-email', label: 'Email Address', type: 'email', placeholder: 'jane@example.com', auto: 'email' }, { id: 'contact-subject', label: 'Subject', type: 'text', placeholder: 'Program inquiry, feedback, etc.', auto: 'off' }].map((f) => (
                      <div key={f.id}>
                        <label htmlFor={f.id} className="block font-600 mb-2 text-base" style={{ color: C.navy }}>{f.label}</label>
                        <input id={f.id} type={f.type} placeholder={f.placeholder} autoComplete={f.auto}
                          ...
                          style={{ border: C.inputBorder, backgroundColor: C.white, color: C.navy, minHeight: '48px' }}
                          onFocus={(e) => (e.currentTarget.style.borderColor = hc ? '#000000' : '#2563eb')}
                          onBlur={(e) => (e.currentTarget.style.borderColor = hc ? '#000000' : '#cbd5e1')} />
                      </div>
                    ))}
                    <div>
                      <label htmlFor="contact-message" className="block font-600 mb-2 text-base" style={{ color: C.navy }}>Message</label>
                      <textarea id="contact-message" rows={4} placeholder="How can we help you?"
                        ...
                        style={{ border: C.inputBorder, backgroundColor: C.white, color: C.navy, fontFamily: 'inherit' }}
                        onFocus={(e) => (e.currentTarget.style.borderColor = hc ? '#000000' : '#2563eb')}
                        onBlur={(e) => (e.currentTarget.style.borderColor = hc ? '#000000' : '#cbd5e1')} />
                    </div>
                    <button type="submit" className="gov-focusable w-full text-base font-700 rounded-lg transition-colors"
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-10">
            <div className="md:col-span-2">
              <h2 className="text-lg font-700 mb-2" style={{ color: hc ? '#ffffff' : '#93c5fd' }}>West Kootenay Parks &amp; Recreation</h2>
              <p className="text-sm" style={{ color: 'rgba(255,255,255,0.55)' }}>Your community. Your parks.</p>
              <p className="text-sm mt-2" style={{ color: 'rgba(255,255,255,0.45)' }}>Serving the West Kootenays since 1980.</p>
            </div>
            <div>
              <h3 className="text-xs font-700 uppercase tracking-widest mb-4" style={{ color: hc ? '#ffffff' : '#93c5fd' }}>Quick Links</h3>
            ...
            <div>
              <h3 className="text-xs font-700 uppercase tracking-widest mb-4" style={{ color: hc ? '#ffffff' : '#93c5fd' }}>Info</h3>
              <address className="not-italic flex flex-col gap-2">
                ...
                <span className="text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>West Kootenay, BC</span>
              </address>
            </div>
          </div>
